...
enum CARDS { _2=2, _3, _4, _5, _6, _7, _8, _9, _T, _J, _Q, _K, _A };
...
class Hand {
    ...
    public:
        Hand(const std::vector<CARDS>& _hand, int _bid) {
            ...
            if(occurances.size()==5 || occurances.size()==4 || occurances.size()==1) {
                type = (occurances.size()==5) ? HIGH_CARD : 
                       (occurances.size()==4) ? ONE_PAIR  :
                                                FIVE_OF_A_KIND;
            } else if(occurances.size()==3) {
